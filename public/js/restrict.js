$(function () {
    ...
    //IMG BUTTONS TO UPLOAD
    $("#btn_upload_course_img").change(function() {
        uploadImg($(this), $("#course_img_path"), $("#course_img"));
    });

    $("#btn_upload_member_img").change(function () {
        uploadImg($(this), $("#member_img_path"), $("#member_img"));
    });
    ...
    $("#form_message").submit(function () {
        ...
        $.ajax({
            type: "POST",
            url: BASE_URL + "restrict/ajax_save_message",
            dataType: "json",
            data: $(this).serialize(),
            beforeSend: function () {
                clearErrors();
        ...
        $(".btn-edit-member").click(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#member_img_path").attr("src", response["img"]["member_img"]);
                    $("#modal_member").modal();
                }
            });
    ...
    function active_btn_messages() {
        $(".btn-view-message").click(function () {

            $.ajax({
                type: "POST",
                url: BASE_URL + "restrict/ajax_get_message_data",
                dataType: "json",
                data: { "message_id": $(this).attr("message_id") },
                success: function (response) {
                    clearErrors();
            ...
            swal.fire({
                ...
                .then((result) => {
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: BASE_URL + "restrict/ajax_delete_message_data",
                            dataType: "json",
                            data: { "message_id": message_id.attr("message_id") },
                            success: function (response) {
                                swal.fire("Sucesso!", "Ação executada com sucesso", "success");
                                dt_messages.ajax.reload();
                            }
                        });
                    }
                });
    ...
    var dt_messages = $("#dt_messages").DataTable({
        "oLanguage": DATATABLE_PTBR,
        "autoWidth": false,
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": BASE_URL + "restrict/ajax_list_messages",
            "type": "POST"
        },
        "columnDefs": [
            { targets: "no-sort", orderable: false },
